...
export default function PropertyDetailClient({ property }: { property: PropertyModel }) {
  const { t } = useLang();

  const features = property.features as { label: string; value: string }[];
    ...
    <main className="min-h-screen bg-[var(--clr-surface)]">
      <div className="max-w-[1600px] mx-auto px-8 md:px-12 py-10">
        ...
        <div className="flex items-center gap-2 text-xs text-[var(--clr-text-secondary)] mb-6">
          <Link href="/" className="hover:text-[var(--clr-primary)] transition-colors">{t("prop_breadcrumb_home")}</Link>
          <span>/</span>
          <Link href="/properties" className="hover:text-[var(--clr-primary)] transition-colors">{t("prop_breadcrumb_properties")}</Link>
          <span>/</span>
        ...
        <div className="flex flex-col lg:flex-row gap-10">
            ...
            <div className="mt-8">
              <div className="flex items-start justify-between gap-4 mb-2">
                ...
                <span className={`flex-shrink-0 text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full ${
                  ...
                  {property.status === "sale" ? t("filter_sale") : t("filter_rent")}
                </span>
              </div>
              ...
              {/* Features table */}
              <h2 className="font-bold text-[var(--clr-text)] text-lg mb-4">{t("prop_details_title")}</h2>
          ...
          <div className="w-full lg:w-80 flex-shrink-0">
            <div className="bg-[var(--clr-bg)] rounded-2xl border border-[var(--clr-border)] shadow-sm p-6 sticky top-6">
              <p className="text-3xl font-black text-[var(--clr-primary)] mb-1">
                {formatPrice(property.priceAmount, property.currency)}
              </p>
              <p className="text-[var(--clr-text-secondary)] text-xs mb-6">
                {property.status === "sale" ? t("prop_purchase_price") : t("prop_monthly_rent")}
              </p>

              {/* Quick stats */}
              <div className="grid grid-cols-3 gap-3 mb-6">
                {[
                  { label: t("prop_beds"), value: property.beds },
                  { label: t("prop_baths"), value: property.baths },
